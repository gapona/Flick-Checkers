...
const en = {
  ...
  tutSkip: 'Skip',
  tutRetry: 'Tap anywhere to try again.',
  tutNext: 'Next',
  tutFinish: 'Finish',
  ...
  coachDone: 'Got it',
  coachSkip: 'Skip',
  // A tour with no way back is a tour you cannot re-read a card of, and it was reported as missing
  // twice in one session. Shown from the second step on — a Back on the first card points at nothing.
  coachBack: 'Back',
  ...
  coachTutorialTitle: 'Learn the shot',
  coachTutorialBody: 'Six short lessons on a real board, one shot each. If you have never played this, start here.',
  ...
  coachTurnTwoBody: 'The border here is the colour of the discs whose turn it is. One shot each — a knockout does NOT buy another. Knock one of your OWN off and the other player shoots twice, however many you lost. Five rounds, one per branch of arms; first to three wins.',
  ...
  helpFriendB: 'A match is five rounds, one for each branch of arms, and the first to three takes it. Whose shot it is shows in the capsule under the board, bordered in that side’s own disc colour — the board never flips, so the colour is the only thing that says which discs are yours. A knockout does not buy another shot; knocking one of your OWN off gives the other player two, and losing two at once is still only two.',
...
const es: Record<StringKey, string> = {
  ...
  tutSkip: 'Saltar',
  tutRetry: 'Toca donde sea para volver a intentarlo.',
  tutNext: 'Siguiente',
  tutFinish: 'Terminar',
  ...
  coachTutorialTitle: 'Aprende el tiro',
  coachTutorialBody: 'Seis lecciones cortas en un tablero real, un tiro cada una. Si nunca has jugado a esto, empieza aquí.',
  ...
  coachTurnTwoBody: 'El borde tiene el color de las fichas a las que les toca. Un tiro cada uno: echar una ficha NO da otro tiro. Si echas una de las TUYAS, el otro jugador tira dos veces, pierdas las que pierdas. Cinco rondas, una por arma; gana quien llegue a tres.',
  ...
  helpFriendB: 'Un combate son cinco rondas, una por cada arma, y gana quien llegue a tres. De quién es el turno se ve en la cápsula bajo el tablero, con el borde del color de las fichas de ese bando: el tablero no gira, así que el color es lo único que dice cuáles son tuyas. Echar una ficha rival no da otro tiro; echar una TUYA le da dos al otro jugador, y perder dos a la vez sigue siendo solo dos.',
